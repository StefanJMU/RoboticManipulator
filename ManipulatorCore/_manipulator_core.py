...
class ManipulatorCore:
    ...
    def inverse_arm_matrix(self):
        return self._calculate_inverse_arm_matrix().detach().numpy()
    ...
    def linear_link_velocity(self):
        ...
        return self._self.calculate_linear_link_velocities()
    ...
    def angular_link_velocity(self):
        ...
        return self._self.calculate_angular_link_velocity()
        ...
        for i in range(len(self.joints)):
            ...
            for j in range(3):
                grads[j, :i + 1] = torch.autograd.grad(base_com[j],
                                                      variables[:i + 1],
                                                      retain_graph=(i != 2))
            linear_velocities[:, i] = torch.sum(grads * joint_speeds, axis=1)  #broadcasting; an unsqueeze might be required
    ...
    def _calculate_angular_link_velocity(self, joint_speeds):
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.joints)) :
                ...
                angular_velocities[:, i] = torch.sum(base_rotation_axes[:, : i + 1] * joint_speeds[:i + 1] * revolute_mask[: i + 1], axis=1)
